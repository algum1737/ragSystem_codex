...
- runbook과 script를 모두 추가하는 방식으로 결정했다.
- `scripts/run-concise-eval-smoke.sh`를 추가했다.
- `docs/manual-deployment-guide.md`에 `Concise Eval Smoke` 섹션을 추가했다.
- 서버 `/opt/ragSystem_codex/scripts/run-concise-eval-smoke.sh`에 script를 반영했다.
- 서버에서 `STAMP=20260610_runbook_test2 bash scripts/run-concise-eval-smoke.sh`를 실행했다.
- script는 report와 trace를 생성했고, `concise-06` 실패를 감지해 exit code 1로 종료했다.
- 실패는 script 오류가 아니라 통지 수단과 사전/사후 예외를 누락한 실제 triage 대상이다.
- smoke 이후 서버 `/health`는 정상이다.
- 결과 문서는 `docs/references/2026-06-10-concise-eval-runbook-script-result.md`에 기록했다.
...
- Pre-flight checks: 통과
  ...
  - 서버 smoke 결과 문서 확인
- Automated tests: 통과
  - `bash -n scripts/run-concise-eval-smoke.sh`
  - `.venv/bin/python scripts/validate_concise_eval_cases.py eval/concise_test_cases.json`
  - `bash scripts/validate-docs.sh`
- Manual/Runtime QA: 통과
  - 서버 script 반영 완료
  - 서버 runbook script가 report와 trace를 생성함
  - 서버 runbook script가 `concise-06` 실패를 감지하고 비정상 종료함
  - 실패 후 서버 `/health`: `{"status":"ok","model":"gemma3:12b"}`
- Skipped/Not Run: full eval은 실행하지 않았다.

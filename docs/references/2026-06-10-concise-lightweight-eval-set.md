...
- 원 질문 top-5 검색에는 `제6조에서 정한 방법`이 포함되지만 제6조의 세부 통지 수단 청크가 항상 직접 포함되지는 않을 수 있다.
- deterministic check는 `명시된 방법`, `약관에 따른 방법`처럼 제6조/약관상 통지 방법을 가리키는 좁은 동의 표현을 허용한다.
- 사전/사후 예외 check는 `미리`, `예측 불가능`, `예측할 수 없`, `통제할 수 없`, `사후 통보`처럼 원문 예외 조건과 같은 의미인 표현을 허용한다.
